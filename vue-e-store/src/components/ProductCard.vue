<template>
    <div class="ml-4 w-56 rounded-xl relative hover:shadow-xl transition hover:-translate-y-1 flex flex-col bg-gray-50">
        ...
        <img class="rounded-xl min-h-48 min-w-48 self-center object-cover" :src="image">
        <div class="flex flex-col mt-1">
            <div class="ml-2 text-emerald-500">{{ price }} ₽</div>
            <div class="ml-2 leading-5 overflow-hidden max-h-10">{{ productName }}</div>
        </div>
        <!--        TODO: Прокинуть объект в addToCart-->
        <div v-if="!isAddedToCart" class=" bg-blue-500 rounded-xl" @click="addToCart()">
